    ...
    fn convert_expression(&mut self, expression: &'a ast::Expression) -> Result<(), ConvertError> {
        match expression {
            ...
            ast::Expression::Function(function) => {
                let anonymous_function = function.as_ref();
                self.work_stack.push(ConvertWork::MakeFunctionExpression {
                    body: anonymous_function.body(),
                    token: anonymous_function.function_token(),
                });

                self.push_function_body_work(anonymous_function.body());
            }
